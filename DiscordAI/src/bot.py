...
def run_discord_bot():
    ...
    async def on_ready():
        ...
        logger.info(f'{client.user} Я снова работаю!')
    ...
    async def help(interaction: discord.Interaction):
        ...
        - `/chat [message]` Чат с ChatGPT!
        - `/draw [prompt]` Ебануть фотку от Dalle2 model
        - `/switchpersona [persona]` Сменить модель ChatGPT
                `random`: Ебануть рандом модель
                `chatgpt`: Дефолт ChatGPT
                `dan`: Dan Mode 11.0, infamous Do Anything Now Mode
                `sda`: Superior DAN has even more freedom in DAN Mode
                `confidant`: Evil Confidant, evil trusted confidant
                `based`: BasedGPT v2, порнуха
                `oppo`: OPPO says exact opposite of what ChatGPT would say
                `dev`: Developer Mode, v2 Developer mode enabled

        - `/private` ChatGPT переключение на приват
        - `/public` ChatGPT переключение на обозревание
        - `/replyall` ChatGPT switch between replyAll mode and default mode
        - `/reset` Очистить историю ChatGPT 
        - `/chat-model` Переключить модель чата
